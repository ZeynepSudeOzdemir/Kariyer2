...
-- --------------------------------------------------
-- Date Created: 12/21/2016 11:12:58
...
GO
IF OBJECT_ID(N'[dbo].[FK_ResumeComment]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[CommentSet] DROP CONSTRAINT [FK_ResumeComment];
GO
...
-- Creating foreign key on [ResumeId] in table 'CommentSet'
ALTER TABLE [dbo].[CommentSet]
ADD CONSTRAINT [FK_ResumeComment]
    FOREIGN KEY ([ResumeId])
    REFERENCES [dbo].[ResumeSet]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating non-clustered index for FOREIGN KEY 'FK_ResumeComment'
CREATE INDEX [IX_FK_ResumeComment]
ON [dbo].[CommentSet]
    ([ResumeId]);
GO
